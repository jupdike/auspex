...
function myTextWrapWalker(doc, parOpts, stylesStringsArr, x, y, width, height, onStyleChange, onWord, onLine) {
  ...
  let dx = 0;
  let dy = 0;
  stylesStringsArr.forEach((ab, ix) => {
    const opts = ab.style;
    let str = ab.str;
    ...
    console.warn('ix:', ix);
    if (ix === stylesStringsArr.length - 1) {
      onLine(true);
    }
  });
...
export function layoutLines(doc, parOpts, stylesStringsArr, x, y, width, height) {
  ...
  function myYieldOneLine(isVeryLastLine) {
    console.warn('isVeryLastLine:', isVeryLastLine);
    if (lastLineBuilder.length < 1) {
      console.warn('return1');
      return;
    }
    ...
    if (!isJustified || isVeryLastLine || noInnerWhitespace) {
      ...
      let padX = 0;
      if (align === 'right') {
        padX = width - docWidthOfString(doc, testTxt.trimEnd(), {});
      } else if (align === 'center') {
        padX = 0.5 * (width - docWidthOfString(doc, testTxt.trimEnd(), {}));
      }
      let dx = 0;
      lastLineBuilder.forEach(ab => {
        ...
        docText(doc, word, x + dx + lastDX + padX, y + lastDY, {lineBreak: false, align: 'left'});
        dx += wordWidth;
      });
      lastLineBuilder = [];
    } else {
      ...
      let indent = 0;
      //console.warn('LLB0:', lastLineBuilder[0]);
      if (isFirst && lastLineBuilder[0].style.textIndent) {
        indent = lastLineBuilder[0].style.textIndent;
      }
      let neededSpace = width - totalFilledSpace - indent;
